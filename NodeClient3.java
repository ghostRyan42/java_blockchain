...
public class NodeClient3 {
    ...
    public void start() throws NoSuchAlgorithmException, ClassNotFoundException {
        try (ServerSocket serverSocket = new ServerSocket(port)) {
            ...
            while (true) {
                ...
                serverInfo.setIpAddress(clientSocket.getInetAddress().getHostAddress());
    ...
    public void connectToPeer(String host, int peerPort) throws NoSuchAlgorithmException, ClassNotFoundException {
        try {
            ...
            NodeMainInfo myInfo = new NodeMainInfo("localhost", port, myWallet.getPublicKey(),myWallet.name);
    ...
    private static class ClientHandler implements Runnable {
        ...
        public ClientHandler(Socket socket,ObjectOutputStream out , ObjectInputStream in, NodeClient3 node, NodeMainInfo peerInfo) {
    ...
    private void startHttpServer() throws IOException {
        ...
        log("Serveur HTTP démarré sur le port 5500");
    
        // Route principale pour servir le fichier HTML
        server.createContext("/", exchange -> {
            File file = new File("sharable_first.html"); // Fichier HTML à servir
            if (file.exists()) {
    ...
    public static void main(String[] args) throws NoSuchAlgorithmException, IOException {
        ...
        node1.setWalletAmount(5000);
        node1.myWallet.name="KOMARA";
        new Thread(() -> {
            try {
                node1.start();
